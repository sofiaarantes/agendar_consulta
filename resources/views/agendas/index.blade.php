        ...
        <div class="layout-page">
          ...
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Editar Agendas</span></h4>
                {{-- Mensagem de sucesso --}}
                @if(session('success'))
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">Minhas Agendas do Mês</h5>
                        <a href="{{ route('agendas.create') }}" class="btn btn-success btn-sm">Criar Agenda</a>
                    </div>

                    <div class="card-body">
                        <!-- Filtro por dia da semana -->
                        <form action="{{ route('agendas.index') }}" method="GET" class="row g-3 mb-4">
                            <div class="col-md-4">
                                <label for="dia_semana" class="form-label">Filtrar por Dia da Semana</label>
                            ...
                            <div class="col-md-3 d-flex align-items-end">
                                ...
                                <a href="{{ route('agendas.index') }}" class="btn btn-secondary">Limpar</a>
                            </div>
                        </form>
                        ...
                        @if($agendas->isEmpty())
                            <div class="alert alert-info text-center">
                                Nenhuma agenda encontrada para o dia selecionado.
                            </div>
                        @else
                            <table class="table table-bordered text-center align-middle">
                                ...
                                <tbody>
                                    @foreach($agendas as $agenda)
                                        <tr>
                                            <td>{{ ucfirst(\Carbon\Carbon::parse($agenda->created_at)->locale('pt_BR')->translatedFormat('F') )}}</td>
                                            <td>{{ implode(', ', $agenda->dias_semana) }}</td>
                                            <td>{{ $agenda->hora_inicio }}</td>
                                            <td>{{ $agenda->hora_fim }}</td>
                                            <td>{{ $agenda->duracao_consulta }} min</td>
                                            <td>
                                                <a href="{{ route('agendas.edit', $agenda->id) }}" class="btn btn-sm btn-primary">Editar</a>
                                                <form action="{{ route('agendas.destroy', $agenda->id) }}" method="POST" style="display:inline-block;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-sm btn-danger"
                                                        onclick="return confirm('Tem certeza que deseja excluir esta agenda?')">
                                                        Excluir
                                                    </button>
                                                </form>
                                            </td>
