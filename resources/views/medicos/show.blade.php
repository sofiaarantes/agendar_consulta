        ...
        <div class="layout-page">
          ...
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
                <h4 class="fw-bold py-3 mb-4">Horários Disponíveis de {{ $medico->usuario->name }}</h4>
                    ...
                    <div class="card-body">
                        ...
                        @else
                            <table class="table text-center">
                                ...
                                <tbody>
                                    @foreach($horariosDisponiveis as $consulta)
                                        <tr>
                                            <td>{{ $consulta->data }}</td>
                                            <td>{{ $consulta->hora_inicio }}</td>
                                            <td>{{ $consulta->hora_fim }}</td>
                                            <td>{{ $consulta->agenda->medico->clinica }}</td>
                                            <td>
                                                <form action="{{ route('consultas.marcar') }}" method="POST">
                                                    @csrf
                                                    <input type="hidden" name="consulta_id" value="{{ $consulta->id }}">
                                                    <button type="submit" class="btn btn-success btn-sm">Marcar</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
